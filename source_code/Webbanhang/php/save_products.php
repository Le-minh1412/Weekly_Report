...
$products = [
    ...
    [
        ...
        'category' => 'Món mặn',
        'price' => 280000,
        'description' => 'Cơm nấu từ gạo ST25 dẻo, hạt cơm tơi ngon, thịt cua tươi chắc nịch, bếp đảo cho săn hạt cơm, rồi đổ cua đã xào thơm vào, xúc miếng cơm chiên cua đầy đặn có thêm hành phi giòn rụm, món ngon như vậy đảm bảo tranh nhau đến miếng cuối cùng.'
    ],
    [
        'id' => 6,
        'status' => 1,
    ...
    [
        ...
        'category' => 'Món mặn',
        'img' => './assets/img/products/ga-khong-loi-thoat.jpg',
        'price' => 500000,
        'desc' => 'Gà không lối thoát là món gà hấp cách thủy ăn kèm lá chanh, sả, gừng, gia vị đặc biệt tạo nên hương vị hấp dẫn, đậm đà riêng biệt.'
    ],
    [
        'id' => 28,
        'status' => 1,
...
foreach ($products as $p) {
    ...
    if ($stmt->num_rows > 0) {
        echo " ID {$id} đã tồn tại, bỏ qua.<br>";
        continue;
    }

    $stmt = $conn->prepare("INSERT INTO products (id, status, title, img, category, price, description) VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    if ($stmt->execute()) {
        echo " Thêm sản phẩm ID {$p['id']} thành công.<br>";
    } else {
